...
(window._gsQueue || (window._gsQueue = [])).push( function() {
	...
	var _numExp = /(\d|\.)+/g,
		...
		_parseColor = function(color) {
			var info = {};
			info.format = "whoCares";
			if (color === "" || color == null || color === "none") {
				info.color = _colorLookup.transparent;
				return info;
			}
			if (_colorLookup[color]) {
				info.color = _colorLookup[color];
				return info;
			}
			if (typeof(color) === "number") {
				info.color = [color >> 16, (color >> 8) & 255, color & 255];
				return info;
			}
			if (color.charAt(0) === "#") {
				info.format = "hex";
			...
			info.color = color.match(_numExp) || _colorLookup.transparent;
			return info;
		};
	...
	window._gsDefine.plugin({
		...
		init: function(target, value, tween) {
			...
			var p, s, c, pt;
			for (p in value) {
				var info = _parseColor(value[p])
				c = info.color;
				this._firstPT = pt = {_next:this._firstPT, p:p, f:(typeof(target[p]) === "function"), n:p, r:false};
				this._format = info.format;
				s = _parseColor( (!pt.f) ? target[p] : target[ ((p.indexOf("set") || typeof(target["get" + p.substr(3)]) !== "function") ? p : "get" + p.substr(3)) ]() ).color;
				pt.s = Number(s[0]);
		...
		set: function(v) {
			...
			while (pt) {
				if(this._format == "hex") {
					val = (((pt.s + (v * pt.c)) << 16) | ((pt.gs + (v * pt.gc)) << 8) | (pt.bs + (v * pt.bc))).toString(16);
